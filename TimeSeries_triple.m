function []=TimeSeries_triple(X_DateTime,Y_Left,Y_Right,Y_Right_2,Color_L,Color_R,Color_R_2,t1,t2,fontsize,Y_Name,Ind_XTickLabel)
% Script Name: TimeSeries_triple.m
...
if ~isempty(Y_Right_2)
    plot(X_DateTime,Y_Right_2,'Color',Color_R_2,'Linewidth',1)
end
...
% initialize the x ticklabels
tick_labels = cell(1, numel(ixmajor));

for i = 1:numel(ixmajor)
    if months(i) == 1 % if Jan
        tick_labels{i} = 'Jan'; 
    elseif months(i) == 7 % if Jul
        tick_labels{i} = 'Jul';
    end
end
...
if Ind_XTickLabel ==0
    ...
    % add the Year under the xtick
    hold on;
    for i = 1:numel(ixmajor)-1
        if years(i) == years(i+1)
            % if they are belong to the same year in the neighborhood
